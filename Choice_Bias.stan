...
real<lower=0,upper=1> w[J];
real<lower=0,upper=1> bias_mean;
real<lower=0,upper=1> bias_std;
real bias_subj[J];
...
loglikdir_pre[n,j]=0;
  ...
  if (choice[n,j]>.9) {
 loglikdir_bias = log(w[j]/(1-w[j]));
 } else {
 loglikdir_bias = log((1-w[j])/w[j]);
 }
 
 loglikdir_total = loglikdir_pre[n,j] + loglikdir_post[n,j] + loglikdir_bias;
...
loglik_end=loglikC;
 ...
 P_all_combined[n,j] = inv_logit(loglik_end);
}
}
}
}
 ...
 sd_m~uniform(0,10);
 mu_m ~ normal(0, 1);
 bias_mean~ normal(.5, 1);
bias_std~ normal(0, 10);
