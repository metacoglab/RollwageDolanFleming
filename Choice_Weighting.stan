...
real  <lower=0, upper=5> w_confirmation[J];
real  <lower=0> mu_confirmation;
real  <lower=0>  sigma_confirmation;

real  <lower=0, upper=5> w_disconfirmation[J];
real  <lower=0> mu_disconfirmation;
real  <lower=0>  sigma_disconfirmation;
...
loglikC = loglikC_pre;
 ...
 if (((Xpost[n,j] > 0) && (choice[n,j] > .9)) || ((Xpost[n,j] < 0) && (choice[n,j] < .1))){
	loglikC = loglikC_pre + w_confirmation[j]*loglikC_post[n,j];
} else {
loglikC= loglikC_pre + w_disconfirmation[j]*loglikC_post[n,j];
}
...
mu_confirmation~normal(1,1);
sigma_confirmation~normal(0,10);
mu_disconfirmation~normal(1,1);

sigma_disconfirmation~normal(0,10);
...
w_confirmation[j]~normal(mu_confirmation,sigma_confirmation);
w_disconfirmation[j]~normal(mu_disconfirmation,sigma_disconfirmation);
